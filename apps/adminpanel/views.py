...
def admin_dashboard(request):
    dossiers_alerte = Dossier.objects.filter(created_at__lte=timezone.now() - datetime.timedelta(days=45))
    dossiers_alerte = [d for d in dossiers_alerte if not d.is_complete()]
    ctx = {
        ...
        'recent_dossiers': Dossier.objects.select_related('client').order_by('-created_at')[:5],
...
def admin_properties(request):
    ...
    return render(request, 'adminpanel/properties/list.html', {'properties': properties})
...
def admin_promotions(request):
    ...
    return render(request, 'adminpanel/promotions/list.html', {'promotions': promotions})
...
def admin_promotion_create(request):
    if request.method == 'POST':
        pr = Promotion(
            ...
            badge_color=request.POST.get('badge_color', '#C1121F'),
...
def admin_clients(request):
    ...
    return render(request, 'adminpanel/clients/list.html', {'clients': clients})
...
def admin_client_create(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(username=username).exists():
            ...
            return render(request, 'adminpanel/clients/form.html', {'action': 'Créer'})
        ...
        messages.success(request, f"Client créé — Identifiant : {username} | Mot de passe : {password}")
...
def admin_client_edit(request, pk):
    u = get_object_or_404(User, pk=pk, role='client')
    if request.method == 'POST':
        ...
        return redirect('admin_clients')
    return render(request, 'adminpanel/clients/form.html', {'client': u, 'action': 'Modifier'})
...
def admin_etapes_globales(request):
    ...
    total_tech = sum(e.percentage for e in etapes_tech)
    total_morc = sum(e.percentage for e in etapes_morc)
    return render(request, 'adminpanel/etapes/list.html', {
...
def admin_etape_globale_create(request):
    if request.method == 'POST':
        ...
        EtapeGlobale.objects.create(
            name=request.POST.get('name'), type=type_, percentage=pct,
            order=int(request.POST.get('order', 0)),
            description=request.POST.get('description', ''),
        )
...
def admin_dossier_create(request):
    ...
    if request.method == 'POST':
        ...
        prop_id = request.POST.get('property_link')
        if prop_id:
            d.property_link_id = prop_id
        ...
        DossierHistorique.objects.create(
            dossier=d,
            message="Dossier créé — cochage automatique planifié (1 étape/semaine sauf dernière)",
            created_by=request.user
        )
        messages.success(request, f"Dossier {d.reference} créé. Cochage automatique activé.")
        return redirect('admin_dossier_detail', pk=d.pk)
    return render(request, 'adminpanel/dossiers/form.html', {
        'clients': clients, 'properties': properties, 'action': 'Créer'
    })
...
def admin_dossier_detail(request, pk):
    ...
    return render(request, 'adminpanel/dossiers/detail.html', {
        ...
        'current_morc': current_morc,
        'morc_locked': morc_locked,
        'last_tech_id': last_tech_id,
        'last_morc_id': last_morc_id,
    })
...
def admin_etape_toggle(request, dossier_pk, etape_pk):
    ...
    if etape.type == 'technique':
        prog = dossier.get_progression_technique()
        total = dossier.get_total_technique()
    else:
        prog = dossier.get_progression_morcellement()
        total = dossier.get_total_morcellement()
    DossierHistorique.objects.create(
        dossier=dossier,
        message=f"Étape '{etape.name}' {action} — avancement {etape.get_type_display()} : {prog}%",
        created_by=request.user
    )
...
def admin_dossiers_alerte(request):
    ...
    tous = Dossier.objects.filter(created_at__lte=seuil).select_related('client')
    ...
    if request.GET.get('export') == 'csv':
        ...
        writer.writerow([
            'Référence', 'Intitulé', 'Client', 'Téléphone', 'Email',
            'Superficie (m²)', 'Date paiement', 'Jours existence',
            'Avancement Technique (%)', 'Avancement Morcellement (%)',
        ])
        for d in dossiers:
            writer.writerow([
                d.reference,
...
def admin_reservations(request):
    reservations = Reservation.objects.select_related('property_link', 'promotion_link').all()
    return render(request, 'adminpanel/reservations/list.html', {'reservations': reservations})
...
def admin_chatbot(request):
    knowledges = ChatbotKnowledge.objects.all()
    unknowns = ChatbotUnknown.objects.all()[:50]
    return render(request, 'adminpanel/chatbot/list.html', {'knowledges': knowledges, 'unknowns': unknowns})
...
def admin_chatbot_create(request):
    if request.method == 'POST':
        ChatbotKnowledge.objects.create(
            ...
            question_example=request.POST.get('question_example'),
